...
void PoolBall::createMaterial()
{
  ...
  material["exponent"]->setFloat(1024.0f);
...
//-----------------------------------------------------------------------------
// Radical inverse sampling pattern
// Fills 2D space in a way that has symmetry on frames 1, 5, 9, 33, ...
...
float4 compute_jitter( unsigned int& sequence )
{
  float x = 0, y = 0;
...
bool CookScene::keyPressed(unsigned char key, int x, int y)
{
	switch (key) {
	case ' ':
